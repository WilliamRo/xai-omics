...
from tes.tes_config import TESConfig as Hub
...
th.config_dir()
th.data_dir = os.path.join(ROOT, '61-TES/data')
...
th.input_shape = [th.slice_num] + [th.xy_size, th.xy_size, 1] \
  if th.slice_num != 1 else [th.xy_size, th.xy_size, 1]
...
def activate():
  ...
  if th.train:
    ...
    assert isinstance(test_set, TESSet)
