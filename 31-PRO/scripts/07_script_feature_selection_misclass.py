...
from sklearn.model_selection import train_test_split, RepeatedKFold, GridSearchCV, cross_val_predict, StratifiedKFold
...
def get_data(data_path, data_linc_path, label_name):
...
def cal_misclassification_count(misclassified_lists):
	flattened_list = list(chain(*misclassified_lists))
	count_dict = Counter(x for x in flattened_list)
...
def traditional_method_cv(x, y, n_splits=1, auc_threshold=0.75, model_type='lr'):
	...
	stratified_kfo = StratifiedKFold(n_splits=n_splits, shuffle=True)
	for fold_i, (train_index, test_index) in enumerate(stratified_kfo.split(x, y)):
	...
	plt.fill_between(mean_fpr, tprs_lower, tprs_upper, color='gray', alpha=0.05)
	...
	if flag:
		...
		plt.plot([0, 1], [0, 1], linestyle='--', lw=2, color='r', label='Diag', alpha=.8)
		plt.rcParams['backend'] = 'TkAgg'
		plt.rcParams['font.sans-serif'] = ['SimHei']
		plt.ylim(0, 1)
		plt.xlim(0, 1)
...
if __name__ == '__main__':
	...
	# Input
	work_dir = r'../data'
	data_file = r'features_combined_395_aorta.xlsx'
	label_file = r'label_395.xlsx'
	...
	save_path = r'misclassified_' + label_file.split('_')[-1]
	...
	pca_components = 6
	auc_threshold = 0.95
	model_repeat, selection_repeat = 10, 10
	t_test_method = [t_test_feature, t_test_feature2][0]
	select_method = ['lasso', 'pca'][0]
	model_type = ['lr', 'xgb', 'svm', 'rf'][0]
	...
	for s_index in range(selection_repeat):
		for i in range(model_repeat):
			_, misclassified_list, metric_list, acc_list, threshold_list = traditional_method_cv(
			...
			misclassified_lists.append(misclassified_list)
			print(f'[{i + 1} / {model_repeat}]')

	count = cal_misclassification_count(misclassified_lists)
	...
	print(f'[{select_method} --- {model_type} --- Repeat number: {model_repeat}]')
	...
	pids = raw_data['pids'].to_list()
	lab = raw_data['lab'].to_list()
	sheet.append(['PIDS', 'Count of misclassification', 'GroundTruth'])
	for i in range(len(input_data)):
		value = count[i] if i in count.keys() else 0
		sheet.append([pids[i], value, lab[i]])
