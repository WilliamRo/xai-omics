...
if __name__ == '__main__':
  MIA_dir = r'..\..\data\BA & MIA\radioimage\MIA'
  save_path_structure = r'D:\Gordon\project_for_learning\xai-alfa\70-ANALYST3D\data\npy_data\MIA_structure.npy'
  save_path_mask = r'D:\Gordon\project_for_learning\xai-alfa\70-ANALYST3D\data\npy_data\MIA_mask.npy'
  wc = -600.0
  ww = 1200.0

  mask_MIA = [file for file in os.listdir(MIA_dir) if '.nii.gz' in file]
  marked_MIA = [[file.split('-')[0], file.split('-')[1].split('.')[0]]
                for file in mask_MIA]

  # Get the structure
  # The shape of the datas is [Patient, Slice, H, W]
  datas = []
  for dir in tqdm(marked_MIA):
    all_dir = os.path.join(MIA_dir, dir[0], dir[1])
  ...
  # Get the mask
  mask_dir = os.path.join(MIA_dir, 'uncompress')
  mask_MIA = os.listdir(mask_dir)
  masks = [read_nii_with_medpy(os.path.join(mask_dir, mask_file))
           for mask_file in mask_MIA]
