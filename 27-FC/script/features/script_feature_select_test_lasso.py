...
if __name__ == '__main__':
	data_dir = r'../../data'
	data_name = r'data_gt_256_suv.xlsx'
	...
	for t in range(2):
		...
		for select_method in ['lasso', 'pca']:
			if select_method == 'pca': continue
			for model_type in ['lr', 'xgb', 'svm', 'rf']:
				...
				mean_list_dict[f'{t}-{select_method}-{model_type}'] = mean_list
				std_list_dict[f'{t}-{select_method}-{model_type}'] = std_list
	...
	workbook.save(os.path.join(
		...
		data_name.split('.xlsx')[0] + '_lasso.xlsx'))
