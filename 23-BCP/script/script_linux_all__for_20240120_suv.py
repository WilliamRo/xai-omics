...
if __name__ == '__main__':
	...
	output_folder_name = 'outputs_20240120_suv'
	raw_folder_name = 'raw_20240120_suv'

	for pid in pids:
		cmd = cmd_template.copy()
		save_dir = os.path.join(data_dir, output_folder_name, pid)
		tmp_dir = os.path.join(save_dir, 'tmp')
		os.makedirs(tmp_dir, exist_ok=True)
		src_dir = os.path.join(data_dir, raw_folder_name, pid)

		# Segmentation
		
		cmd.append(f'{fastsufer_dir}/run_fastsurfer.sh --sd $SUBJECTS_DIR --sid {pid} --t1 {src_dir}/raw_mr.nii --seg_only --device cuda:1 --ignore_fs_version --threads 6 --parallel')
		...
		# Coregister
		# av45 and av1451
		if os.path.exists(os.path.join(src_dir, 'raw_ab_pet.nii')):
			cmd.append(f'matlab -nodisplay -nosplash -nodesktop -r "addpath(\'{spm_dir}\');coregister(\'{src_dir}/raw_ab_pet.nii\',\'{save_dir}/fastsurfer_mr.nii\');exit;"')
			cmd.append(f'mv {src_dir}/rraw_ab_pet.nii {save_dir}/cor_ab_pet.nii')

		if os.path.exists(os.path.join(src_dir, 'raw_tau_pet.nii')):
			cmd.append(f'matlab -nodisplay -nosplash -nodesktop -r "addpath(\'{spm_dir}\');coregister(\'{src_dir}/raw_tau_pet.nii\',\'{save_dir}/fastsurfer_mr.nii\');exit;"')
			cmd.append(f'mv {src_dir}/rraw_tau_pet.nii {save_dir}/cor_tau_pet.nii')
