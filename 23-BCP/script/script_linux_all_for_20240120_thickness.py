...
if __name__ == '__main__':
	...
	output_thickness_name = 'outputs_20240120_thickness'
	raw_folder_name = 'raw_20240120'

	for pid in pids:
		cmd = cmd_template.copy()
		save_dir = os.path.join(data_dir, output_folder_name, pid)
		save_thickness_dir = os.path.join(data_dir, output_thickness_name, pid)
		...
		cmd.append(f'{fastsufer_dir}/run_fastsurfer.sh --sd $SUBJECTS_DIR --sid {pid} --t1 {src_dir}/raw_mr.nii --device cuda --ignore_fs_version --threads 6 --parallel')
		...
		cmd.append(f'cp $SUBJECTS_DIR/{pid}/stats/lh.aparc.DKTatlas.mapped.stats {save_thickness_dir}/lh.aparc.stats')
		cmd.append(f'cp $SUBJECTS_DIR/{pid}/stats/rh.aparc.DKTatlas.mapped.stats {save_thickness_dir}/rh.aparc.stats')
